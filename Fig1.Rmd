...
rect(xleft=rep(par("usr")[2],ncol(meanZlig)),
     ...
     xpd=NA,border=NA,
     col=sequential_hcl(100,palette="inferno",rev=T)[cut(scoreDElig,100)])
...
rect(xleft=rep(par("usr")[2],ncol(meanZct)),
     ...
     xpd=NA,border=NA,
     col=sequential_hcl(100,palette="inferno",rev=T)[cut(scoreDEct,100)])
...
```{r echo=F,message=F,warning=F,fig.height=5,fig.width=9}
load("~/Dropbox/GDB/CMapCorr/Fig1_rep.RData")
...
mtext(lig16,at=seq(2,by=3,length.out=16),
      line=rep(c(0,0.7),2),col=lig_col,cex=0.8)
