...
```{r DE_dot_TNF, echo=F,message=F,warning=F,fig.height=8,fig.width=4.3}
LIG <- "TNF"
temp_cmapgenes <- lvl4_data@rdesc[rownames(FDR_lig)[FDR_lig[,LIG] <= 0.1],"pr_gene_symbol"]
temp_genes <- Reduce(union,nn_DE[[LIG]])
temp_cmapgenes <- temp_cmapgenes[temp_cmapgenes %in% temp_genes]
...
```{r noTS_diffexp_meanLFC, echo=F,message=F,warning=F,fig.height=5,fig.width=9, eval=F}

plot(unlist(sapply(mean_lfc,"[","nts_lfc")),
     -log10(unlist(sapply(mean_lfc,"[","nts_fdr"))),
     pch=".")
