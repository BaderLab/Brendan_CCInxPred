...
if (file.exists("~/Dropbox/GDB_archive/CMapCorr_files/200730_lvl3_breastonly.RData")) {
  ...
  rm(lig16,ct14)
  temp_cdesc <- lvl3_data@cdesc[lvl3_data@cdesc$cell_id %in% ctBR,]
  lig29 <- Reduce(intersect,tapply(temp_cdesc$pert_iname,temp_cdesc$cell_id,c))
  temp_cdesc <- temp_cdesc[temp_cdesc$pert_iname %in% lig29,]
  temp_cdesc_ctrl <- lvl3_data_ctl@cdesc[lvl3_data_ctl@cdesc$rna_plate %in% unique(temp_cdesc$rna_plate),]
  BRmat <- cbind(lvl3_data@mat[,rownames(temp_cdesc)],
                 lvl3_data_ctl@mat[,rownames(temp_cdesc_ctrl)])
  BRcdesc <- rbind(temp_cdesc,temp_cdesc_ctrl)
  save(BRmat,BRcdesc,lig29,ctBR,file="~/Dropbox/GDB_archive/CMapCorr_files/200730_lvl3_breastonly.RData")
}

# ^ UMAP ----
if (!file.exists("~/Dropbox/GDB_archive/CMapCorr_files/200730_lvl3_breastonly_umap.RData")) {
  ...
  save(BRumap,file="~/Dropbox/GDB_archive/CMapCorr_files/200730_lvl3_breastonly_umap.RData")
}
# load("~/Dropbox/GDB_archive/CMapCorr_files/200730_lvl3_breastonly_umap.RData")
...
save(scores_nobal,file="~/Dropbox/GDB_archive/CMapCorr_files/200730_probs_mixall_breast_lvl3_nobal.RData")
