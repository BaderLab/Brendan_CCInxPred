...
if (file.exists("../CMapCorr_files/200427_lvl4_mixall.RData")) {
  temp <- load("../CMapCorr_files/200427_lvl4_mixall.RData")
} else {
  source("200427_lvl4_mixall.R")
}

tempdf <- as.data.frame(
  rbind(training=table(lvl4_data@cdesc[trainIDs,"pert_iname"]),
        testing=table(lvl4_data@cdesc[testIDs,"pert_iname"]),
        total=table(lvl4_data@cdesc[c(trainIDs,testIDs),"pert_iname"]))
)
kable(tempdf,format="html")  %>%
  kable_styling()

temp_confusion <- table(true=lvl4_data@cdesc[testIDs,"pert_iname"],
...
[Random forest model source code](https://github.com/BaderLab/Brendan_CCInxPred/blob/master/200427_lvl4_mixall.R)  

```{r RF_balanced,echo=FALSE,message=FALSE,warning=FALSE,fig.height=7,fig.width=9,fig.show='hold'}
if (file.exists("../CMapCorr_files/200427_lvl4_mixall_balanced.RData")) {
  temp <- load("../CMapCorr_files/200427_lvl4_mixall_balanced.RData")
} else {
  source("200427_lvl4_mixall.R")
}
...
if (file.exists("../CMapCorr_files/200427_lvl4_mixall_balanced_saturated.RData")) {
  temp <- load("../CMapCorr_files/200427_lvl4_mixall_balanced_saturated.RData")
} else {
  source("200427_lvl4_mixall.R")
}
...
```{r UMAP_ct,echo=FALSE,message=FALSE,warning=FALSE,fig.height=5.5,fig.width=9,fig.show='hold'}
if (exists("UMAPmixall")) {
  temp <- NULL
} else if (file.exists("../CMapCorr_files/200427_lvl4_UMAPmixall.RData")) {
  load("../CMapCorr_files/200427_lvl4_UMAPmixall.RData") 
} else {
  source("200426_lvl4_pca_umap.R")
}
temp_allcol <- as.factor(lvl4_data@cdesc[rownames(UMAPmixall$layout),"cell_id"])
...
for (L in names(sort(acc_balanced,decreasing=T))) {
  ...
  temp_col <- temp_allcol[lvl4_data@cdesc[rownames(UMAPmixall$layout),"pert_iname"] == L]
  points(UMAPmixall$layout[lvl4_data@cdesc[rownames(UMAPmixall$layout),"pert_iname"] == L,],
...
temp_mean <- sapply(names(sort(acc_balanced,decreasing=T)),function(L)
  rowMeans(lvl4_data@mat[,lvl4_data@cdesc$pert_iname == L]),
  simplify=T)
temp_sd <- sapply(names(sort(acc_balanced,decreasing=T)),function(L)
  apply(lvl4_data@mat[,lvl4_data@cdesc$pert_iname == L],1,sd),
  simplify=T)
...
text(scClustViz::spreadLabels2(
  ...
  as.vector(temp_sd)[as.vector(temp_meansd)],
  gene_info[rep(rownames(temp_sd),ncol(temp_sd))[as.vector(temp_meansd)],"pr_gene_symbol"],
  str.cex=0.8),cex=0.8,
  labels=gene_info[rep(rownames(temp_sd),ncol(temp_sd))[as.vector(temp_meansd)],"pr_gene_symbol"],
...
if (file.exists("../CMapCorr_files/200524_lvl4_mixall_balanced_leave1out_results.RData")) {
  temp <- load("../CMapCorr_files/200524_lvl4_mixall_balanced_leave1out_results.RData")
} else {
  source("200524_lvl4_mixall_leave1out.R")
}
...
test_hits <- vector("list",sum(lvl4_data@cdesc$pert_iname %in% lig15))
names(test_hits) <- rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname %in% lig15]
...
acc_leave1out <- sapply(names(test_hits),function(X) 
  sum(test_hits[[X]] == lvl4_data@cdesc[X,"pert_iname"]) / 
    length(test_hits[[X]]))
acc_leave1out_totals <- sapply(lig15,function(X) {
  Y <- rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname == X]
...
acc_leave1out_ct <- sapply(lig15,function(X) {
  sapply(ct14,function(Y) {
    Z <- rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname == X &
                                     lvl4_data@cdesc$cell_id == Y]
...
test_counts <- sapply(lig15,function(X) {
  sapply(ct14,function(Y) {
    sum(lvl4_data@cdesc$pert_iname == X &
          lvl4_data@cdesc$cell_id == Y)
  })
})
...
test_predictions <- sapply(lig15,function(X) {
  sapply(ct14,function(Y) {
    temp <- rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname == X &
                                        lvl4_data@cdesc$cell_id == Y]
...
```{r UMAP_marker_gene,echo=FALSE,message=FALSE,warning=FALSE,fig.height=11,fig.width=9,fig.show='hold'}
if (exists("UMAPmixall")) {
  temp <- NULL
} else if (file.exists("../CMapCorr_files/200427_lvl4_UMAPmixall.RData")) {
  load("../CMapCorr_files/200427_lvl4_UMAPmixall.RData") 
...
temp_allcol <- temp_allcol[rownames(UMAPmixall$layout)]
layout(rbind(1:3,matrix(4:9,ncol=3),matrix(10:15,ncol=3)),
       heights=c(3,3,1,3,1))
par(mar=c(1.5,1,1,0.5),mgp=c(0,0,0))
for (L in c("TNF","IFNG","BTC")) {
  plot(NA,NA,xlim=range(UMAPmixall$layout[,1]),ylim=range(UMAPmixall$layout[,2]),
       xaxt="n",yaxt="n",xlab="UMAP1",ylab="UMAP2",
  ...
  temp_col <- temp_allcol[lvl4_data@cdesc[names(temp_allcol),"pert_iname"] == L]
  points(UMAPmixall$layout[lvl4_data@cdesc[rownames(UMAPmixall$layout),"pert_iname"] == L,],
         pch=19,col=sequential_hcl(100,palette="inferno",rev=T,alpha=0.7)[temp_col])
}

temp_genes <- c("NFKBIA","STAT1","DUSP4","RELB","PSMB8","ICAM1")
temp_lig <- list("TNF","IFNG","BTC","TNF","IFNG",c("TNF","IFNG"))
par(mar=c(1,1,1,0.5),mgp=c(0,0,0))
temp_par <- par(c("mar","mgp","bty","las"))
for (N in seq_along(temp_genes)) {
  temp_z <- lvl4_data@mat[
    rownames(gene_info)[gene_info$pr_gene_symbol == temp_genes[N]],
    rownames(UMAPmixall$layout)
    ]
  ...
  temp_big <- rownames(UMAPmixall$layout) %in% 
    rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname %in% temp_lig[[N]]]
  
  par(temp_par)
  plot(NA,NA,xlim=range(UMAPmixall$layout[,1]),ylim=range(UMAPmixall$layout[,2]),
       xaxt="n",yaxt="n",xlab="UMAP1",ylab="UMAP2",main=temp_genes[N])
  points(UMAPmixall$layout[!temp_big,],pch=20,
         col=colorspace::diverge_hcl(100, palette = "Purple-Green",alpha=.8)[temp_col])
  points(UMAPmixall$layout[temp_big,],pch=19,
  ...
  temp_bp <- append(temp_bp,
                    sapply(temp_lig[[N]],function(X) 
                      temp_z[names(temp_z) %in% 
                               rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname %in% X]],
                      simplify=F))
...
[Random forest model source code](https://github.com/BaderLab/Brendan_CCInxPred/blob/master/200524_lvl4_mixall_leave1out.R)  
...
temp_col <- unlist(lapply(qualitative_hcl(ceiling(length(lig15) / 2),palette="dark3"),
                          function(X) rep(X,2)))[1:length(lig15)]
names(temp_col) <- lig15
layout(rbind(c(16,1:15)),widths=c(2,rep(1,15)))
...
for (LIG in lig15) {
  ...
  rect(0.25,14.1,0.75,14.5,xpd=NA,col=temp_col[LIG],border="black",
       density=switch(as.character(which(lig15 == LIG) %% 2),'1'=NA,'0'=40))
  temp2 <- par("usr")
}
par(mar=c(0,0,5,0))
...
if (!file.exists("output_figs/TNF.png")) {
  for (LIG in lig15) {
    temp_lig <- rownames(lvl4_data@cdesc)[lvl4_data@cdesc$pert_iname == LIG &
                                            lvl4_data@cdesc$cell_id %in% ct14]
    temp_ct <- factor(lvl4_data@cdesc[temp_lig,"cell_id"],levels=ct14)
    levels(temp_ct) <- names(ct14)
    
    tempZ <- apply(lvl4_data@mat[,temp_lig],1,function(X) lm(X ~ 0 + temp_ct))
    ...
    rownames(temp_heat) <- gene_info[rownames(tempZTtop),"pr_gene_symbol"]
    ...
    png(paste0("output_figs/",LIG,"_sm.png"),width=180,height=90,units="px")
...
temp <- matrix(lig15,nrow=3,byrow=T)
