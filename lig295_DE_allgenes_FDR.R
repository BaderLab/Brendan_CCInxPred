...
FDR_lig <- sapply(lig295,function(LIG)
  p.adjust(pnorm(-abs(
...
BKG_lig <- pbsapply(lig295,function(LIG) {
  temp <- sapply(1:1e3,function(L) 
    p.adjust(pnorm(-abs(
...
save(FDR_lig,pDE_lig,
     file=paste0("~/Dropbox/GDB_archive/CMapCorr_files/lig295_DE_allgenes_lig_FDR.RData"))
...
BKG_ct <- pbsapply(names(ct9),function(CT) {
  temp <- sapply(1:1e3,function(L) 
    p.adjust(pnorm(-abs(
...
pDE_ct <- sapply(ALPHA,function(Z) {
  ...
  sapply(names(ct9),function(CT) {
    sum(BKG_ct[[CT]][,names(which(ALPHA == Z))] >= temp_count[CT]) / nrow(BKG_ct[[CT]])
  })
})

save(FDR_ct,pDE_ct,
     file=paste0("~/Dropbox/GDB_archive/CMapCorr_files/lig295_DE_allgenes_ct_FDR.RData"))
...
FDR_ligct <- sapply(ct9,function(CT) {
  sapply(lig295,function(LIG) {
    p.adjust(pnorm(-abs(
      rowMeans(lvl4_data@mat[,lvl4_data@cdesc$cell_id == CT &
...
save(FDR_ligct,pDE_ligct,
     file=paste0("~/Dropbox/GDB_archive/CMapCorr_files/lig295_DE_allgenes_ligct_FDR.RData"))
...
save(FDR_rep,pDE_rep,
     file=paste0("~/Dropbox/GDB_archive/CMapCorr_files/lig295_DE_allgenes_rep_FDR.RData"))
rm(list=grep("ligct$",ls(),value=T))
rm(list=grep("^temp",ls(),value=T))
