---
title: "Ligand transcriptome perturbations from Connectivity Map"
---
...
temp <- as.data.frame(sapply(lig15,function(X)
...
```{r lvl5_umap, echo=FALSE, message=FALSE, warning=FALSE, fig.height=9, fig.width=9, fig.show='hold'}
if (exists("lvl5umap")) {
} else if (file.exists("../CMapCorr_files/200426_lvl5umap.RData")) {
  load("../CMapCorr_files/200426_lvl5umap.RData") 
} else {
  source("200426_lvl5_pca_umap.R")
}

par(mfrow=c(2,2),mar=c(1.5,1,1,0.5),mgp=c(0,0,0))
plot(lvl5umap$layout,xaxt="n",yaxt="n",pch=".",cex=2,
     main="UMAP from top 50 PCs coloured by cell type",
     xlab="UMAP1",ylab="UMAP2",
     col=qualitative_hcl(
       length(unique(lvl5_data@cdesc$cell_id)),
       palette="dark3",alpha=.7)[as.factor(lvl5_data@cdesc$cell_id)])
plot(lvl5umap$layout,xaxt="n",yaxt="n",pch=".",cex=2,
     main="UMAP from top 50 PCs coloured by ligand",
     xlab="UMAP1",ylab="UMAP2",
     col=qualitative_hcl(
...
} else if (file.exists("../CMapCorr_files/200426_lvl5umap_raw.RData")) {
  load("../CMapCorr_files/200426_lvl5umap_raw.RData") 
} else {
  source("200426_lvl5_pca_umap.R")
}

plot(lvl5umap_raw$layout,xaxt="n",yaxt="n",pch=".",cex=2,
     main="UMAP from all assayed genes coloured by cell type",
     xlab="UMAP1",ylab="UMAP2",
     col=qualitative_hcl(
       length(unique(lvl5_data@cdesc$cell_id)),
       palette="dark3",alpha=.7)[as.factor(lvl5_data@cdesc$cell_id)])
plot(lvl5umap_raw$layout,xaxt="n",yaxt="n",pch=".",cex=2,
     main="UMAP from all assayed genes coloured by ligand",
     xlab="UMAP1",ylab="UMAP2",
     col=qualitative_hcl(
       length(unique(lvl5_data@cdesc$pert_iname)),
       palette="dark3",alpha=.7)[as.factor(lvl5_data@cdesc$pert_iname)])
...
if (exists("ctUMAPraw")) {
} else if (file.exists("../CMapCorr_files/200426_lvl5_ctUMAP.RData")) {
  load("../CMapCorr_files/200426_lvl5_ctUMAP.RData") 
} else {
  source("200426_lvl5_pca_umap.R")
}
...
for (CT in names(ctUMAPraw)) {
  plot(ctUMAPraw[[CT]]$layout,xaxt="n",yaxt="n",pch=20,
       ...
       col=qualitative_hcl(
         ...
         palette="dark3",alpha=.7)[as.factor(lvl5_data@cdesc[rownames(ctUMAPraw[[CT]]$layout),
                                                             "pert_iname"])])
  temp_lig <- lvl5_data@cdesc[rownames(ctUMAPraw[[CT]]$layout),"pert_iname"] %in% lig15
  ...
  points(ctUMAPraw[[CT]]$layout[temp_lig,],pch=20,
         col=qualitative_hcl(length(lig15),palette="dark3",alpha=0.7)[temp_col])
...
```{r lvl5_umap_lig,echo=FALSE,message=FALSE,warning=FALSE,fig.height=7.5,fig.width=9,fig.show='hold'}
if (exists("ligUMAP")) {
} else if (file.exists("../CMapCorr_files/200426_lvl5_ligUMAP.RData")) {
  load("../CMapCorr_files/200426_lvl5_ligUMAP.RData") 
} else {
  source("200426_lvl5_pca_umap.R")
}

par(mfrow=c(5,6),mar=c(1,1,1,0.5),mgp=c(0,0,0))
