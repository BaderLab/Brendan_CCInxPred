...
for (FN in rev(list.files("~/Dropbox/GDB_archive/CMapCorr_files/","^GSEA_lvl4_[A-Z]"))) {
  ...
  NQhits_CT <- pbsapply(10^-allCuts,function(CUT) 
    sapply(Qscore_CT,function(X) mean(X <= CUT)))
  ...
  Mscore_LIG <- pbreplicate(ceiling(max(temp_counts / DS)),{
  ...
  NQhits_LIG <- pbsapply(10^-allCuts,function(CUT) 
    sapply(Qscore_LIG,function(X) mean(X <= CUT)))
  ...
  Mscore_LIGCT <- replicate(ceiling(max(temp_counts / DS)),{
    sapply(temp_ids,function(ID)
  ...
  NQhits_LIGCT <- pbsapply(10^-allCuts,function(CUT) 
    sapply(Qscore_LIGCT,function(X) mean(X <= CUT)))
