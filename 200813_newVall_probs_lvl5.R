...
# new_trainID <- rownames(lvl5_data@cdesc)[lvl5_data@cdesc$cell_id != CT]
# new_trainID_true <- new_trainID[lvl5_data@cdesc[new_trainID,"pert_iname"] == LIG]
...
# 
# all_trainID_true <- sample(rownames(lvl5_data@cdesc)[lvl5_data@cdesc$pert_iname == LIG],
#                            length(new_trainID_true))
# all_trainID_false <- sample(rownames(lvl5_data@cdesc)[lvl5_data@cdesc$pert_iname != LIG],
#                             length(new_trainID_false))
# all_trainID <- sample(c(all_trainID_true,all_trainID_false))
# all_testID <- setdiff(rownames(lvl5_data@cdesc),all_trainID)
# 
# tuning_results <- pbapply::pbsapply(seq(1,201,10),function(X) {
#   rfmodel <- ranger(x=t(lvl5_data@mat[,all_trainID]),
#                     y=lvl5_data@cdesc[all_trainID,"pert_iname"] == LIG,
#                     num.threads=8,num.trees=1e3,probability=T,
#                     min.node.size=10,verbose=F)
#   testResults <- predict(rfmodel,t(lvl5_data@mat[,all_testID]))
...
#   all_out$label <- lvl5_data@cdesc[all_testID,"pert_iname"] == LIG
#   print(X)
...
for (LIG in lig16) {
  ...
  for (CT in ct14) {
    ...
    rfmodel <- ranger(x=t(lvl5_data@mat[,new_trainID]),
                          y=lvl5_data@cdesc[new_trainID,"pert_iname"] == LIG,
                          num.threads=8,num.trees=1e3,probability=T,
                          verbose=F)
    testResults <- predict(rfmodel,t(lvl5_data@mat))
    
    new_out <- as.data.frame(testResults$predictions[,which(colnames(rfmodel$predictions) == "TRUE")])
    rownames(new_out) <- colnames(lvl5_data@mat)
    colnames(new_out) <- "score"
    new_out$label <- lvl5_data@cdesc$pert_iname == LIG
    ...
    all_out <- as.data.frame(testResults$predictions[,which(colnames(rfmodel$predictions) == "TRUE")])
    rownames(all_out) <- colnames(lvl5_data@mat)
    colnames(all_out) <- "score"
    all_out$label <- lvl5_data@cdesc$pert_iname == LIG
...
save(scores_all,scores_new,mccf1_all,mccf1_all_thresh,mccf1_new,mccf1_new_thresh,
     file="~/Dropbox/GDB_archive/CMapCorr_files/200817_newVall_probs_lvl5.RData")
